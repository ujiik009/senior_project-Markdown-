	...
	function regexMD_to_html(text){
		...
		//let fn_highlight = "$('pre code').each(function(i, block) {hljs.highlightBlock(block);});";
		let fn_highlight ="<script type='text/javascript'>hljs.initHighlightingOnLoad(); </script>";
		let template = "<!DOCTYPE html><html><head><meta charset='utf-8'><title>"+document_name+"</title><style type='text/css'>"+link_css+"</style></head><body>{{content}}</body>"+link_jquery+link_highlight+fn_highlight+"</html>";
		var objpatt = {
			 pattH1 		: /(^#{1}? )(.+)?/gm,
			 pattH2 		: /(^#{2}? )(.+)?/gm,
			 pattH3 		: /(^#{3}? )(.+)?/gm,
